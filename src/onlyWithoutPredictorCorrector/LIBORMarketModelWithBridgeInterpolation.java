 ...
 */
package onlyWithoutPredictorCorrector;
 ...
 */
public class LIBORMarketModelWithBridgeInterpolation extends AbstractModel implements LIBORMarketModelInterface {
	...
	public RandomVariableInterface[] getInitialState() {
		double[] liborInitialStates = new double[getNumberOfLibors()];
		for(int timeIndex=0; timeIndex<getNumberOfLibors(); timeIndex++) {
		...
		// Brownian Bridge starts at 0
		initialStateRandomVariable[getNumberOfComponents() - 1] = getRandomVariableForConstant(0.0);
	...
	public RandomVariableInterface[] getDrift(int timeIndex, RandomVariableInterface[] realizationAtTimeIndex, RandomVariableInterface[] realizationPredictor) {
		...
		RandomVariableInterface[]	covarianceFactorSums	= new RandomVariableInterface[covarianceModel.getNumberOfFactors()]; //Last Factor is Bridge Driver
		...
		if(measure == Measure.SPOT) {
			...
			for(int componentIndex=firstLiborIndex; componentIndex<getNumberOfLibors(); componentIndex++) {
				...
				RandomVariableInterface[]	factorLoading   	= covarianceModel.getFactorLoading(timeIndex, componentIndex, realizationAtTimeIndex);
			...
			// Calculate drift for the component componentIndex (starting at firstLiborIndex, others are zero)
			for(int componentIndex=getNumberOfLibors()-1; componentIndex>=firstLiborIndex; componentIndex--) {
				double					periodLength	= liborPeriodDiscretization.getTimeStep(componentIndex);
				RandomVariableInterface libor			= realizationAtTimeIndex[componentIndex];
				RandomVariableInterface oneStepMeasureTransform = getRandomVariableForConstant(periodLength).discount(libor, periodLength);
		...
		//int liborPeriodEndIndex = getLiborPeriodIndex(time) > 0 ? getLiborPeriodIndex(time) + 1 : -getLiborPeriodIndex(time) - 1;
		double liborPeriodEndTime = getLiborPeriod(firstLiborIndex);
		
		/*if(realizationAtTimeIndex[getNumberOfComponents() - 1].getFiltrationTime() != time) {
			try {
				realizationAtTimeIndex[getNumberOfComponents() - 1] = getProcessValue(timeIndex - 1, getNumberOfComponents() - 1);
			} catch (CalculationException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}*/
		drift[getNumberOfComponents() - 1] = realizationAtTimeIndex[getNumberOfComponents() - 1].div(time - liborPeriodEndTime);
	...
	public	RandomVariableInterface[]	getFactorLoading(int timeIndex, int componentIndex, RandomVariableInterface[] realizationAtTimeIndex)
	{
		if(componentIndex < getNumberOfLibors()) {
			RandomVariableInterface[] factorLoadingFromModel  = covarianceModel.getFactorLoading(getTime(timeIndex), getLiborPeriod(componentIndex), realizationAtTimeIndex);
			RandomVariableInterface[] factorLoading = new RandomVariableInterface[getNumberOfFactors()];
			for (int factorIndex = 0; factorIndex < factorLoadingFromModel.length; factorIndex++) {
				factorLoading[factorIndex] = factorLoadingFromModel[factorIndex];
			}
			factorLoading[getNumberOfFactors() - 1] = getRandomVariableForConstant(0.0);
		 	return factorLoading;
		}
		if(componentIndex == getNumberOfComponents() - 1) {
			int		endLiborIndex		= getLiborPeriodIndex(getTime(timeIndex))+1;
			if(endLiborIndex<0) endLiborIndex = -endLiborIndex-1 + 1;
			double endLiborPeriodTime = getLiborPeriod(endLiborIndex);
			double alpha = (getTime(timeIndex + 1) - getTime(timeIndex)) / (endLiborPeriodTime - getTime(timeIndex));
			RandomVariableInterface[] factorLoadingForBridge = new RandomVariableInterface[getNumberOfFactors()];
			Arrays.fill(factorLoadingForBridge, getRandomVariableForConstant(0.0));
			factorLoadingForBridge[getNumberOfFactors() - 1] = getRandomVariableForConstant(1.0).mult(Math.sqrt(1 - alpha)); //TODO: placeholde
			return factorLoadingForBridge;
		}
		throw new IndexOutOfBoundsException("componentIndex " + componentIndex + "is greater than Number of components: " + getNumberOfComponents());
	}
	...
	public int getNumberOfComponents() {
		return getNumberOfLibors() + 1; //Last Component is Brownian Bridge
	}
					...
					for(int componentIndex1 = 0; componentIndex1 < liborPeriodDiscretization.getNumberOfTimeSteps(); componentIndex1++) {
						...
						for(int componentIndex2 = componentIndex1; componentIndex2 < liborPeriodDiscretization.getNumberOfTimeSteps(); componentIndex2++) {
							...
							if(getLiborPeriod(componentIndex1) > getTime(timeIndex)) {
								...
								for(int factorIndex = 0; factorIndex < factorLoadingOfComponent1.length; factorIndex++) {
	...
	public Object clone() {
		try {
			...
			return new LIBORMarketModelWithBridgeInterpolation(getLiborPeriodDiscretization(), getAnalyticModel(), getForwardRateCurve(), getDiscountCurve(), randomVariableFactory, covarianceModel, new CalibrationItem[0], properties);
		} catch (CalculationException e) {
			return null;
		}
	}
	...
	public LIBORMarketModelWithBridgeInterpolation getCloneWithModifiedCovarianceModel(AbstractLIBORCovarianceModel covarianceModel) {
		LIBORMarketModelWithBridgeInterpolation model = (LIBORMarketModelWithBridgeInterpolation)this.clone();
		model.covarianceModel = covarianceModel;
		return model;
	}

	@Override
	public LIBORMarketModelWithBridgeInterpolation getCloneWithModifiedData(Map<String, Object> dataModified) throws CalculationException {
		...
		LIBORMarketModelWithBridgeInterpolation newModel = new LIBORMarketModelWithBridgeInterpolation(liborPeriodDiscretization, forwardRateCurve, discountCurve, covarianceModel, swaptionMarketData, properties);
